...
export default {
  ...
  keys: {
    ...
    sortValue: "ソート値",
    maxRetries: "最大リトライ回数",
    maxRetriesPlaceholder: "1 回の集約リクエストでサブグループを切り替える最大回数（0-5000）",
    subMaxRetries: "サブグループ最大リトライ",
    subMaxRetriesPlaceholder:
      "通常リクエストとアフィニティ降格後のグループ内キー再試行のみを制限します。空欄は追加上限なし、0 は再試行なし（0-500）",
    ...
    codexAffinityHint:
      "OpenAI Responses の集約グループに対する Codex リクエストにのみ適用され、既定では無効です。有効にすると、同じ Codex セッションは安定したサブグループを優先します。そのサブグループが利用不可、またはアフィニティ試行を使い切った場合、残りのサブグループを有効重みに基づいて選択します。ヘッダーは変更せず、アフィニティ降格後は互換性のため Responses encrypted reasoning を除去します。",
    codexAffinityMaxRetries: "Codex アフィニティ最大リトライ",
    codexAffinityMaxRetriesHint:
      "初回リクエストを含むアフィニティの総試行回数。既定値 5 = 初回 1 回 + 最大 4 回のアフィニティ再試行。この回数を使い切ってからサブグループ切り替えに入り、切り替え回数は引き続き最大リトライ回数で制限されます。ほかの 2 項目は初回後のサブグループ切り替えまたはキー再試行のみを数えます。",
    precondition: "前提条件",
    preconditions: "前提条件",
    ...
    maxRequestSizeHint:
      "集約グループのリクエストサイズ制限を設定。このサイズを超えるリクエストは拒否されます（0 = 制限なし）",
    preconditionNotMet: "リクエストが集約グループの前提条件を満たしていません",
    addPrecondition: "前提条件を追加",
    selectPrecondition: "前提条件タイプを選択",
    preconditionValue: "値を入力",
    optional: "オプション",
    testModelTooltip:
    ...
    childGroupUpstreamAutoManaged:
      "子グループのアップストリームURLはシステムによって自動管理され、常に親グループのプロキシエンドポイント（http://127.0.0.1:PORT/proxy/親グループ名）を指します。手動で変更できません",
    weightTooltip:
    ...
    upstreamProxyUrlTooltip:
      "このアップストリーム専用のプロキシサーバーアドレス（http/https/socks5対応）。設定されている場合、このプロキシが優先的に使用されます。空の場合、グループまたはグローバルプロキシ設定が使用されます",
    upstreamProxyUrlPlaceholder: "プロキシなし",
  ...
  logs: {
    ...
    totalRecords: "合計 {total} 件",
    currentPageRange: "{start}-{end} 件を表示",
    currentPageNoRecords: "このページに記録はありません",
    recordsPerPage: "{count}件/ページ",
    previousPage: "前のページ",
    nextPage: "次のページ",
    pageInfo: "{current} / {total} ページ",
    ...
    thinkingTokens: "思考トークン",
    tokenUsageSource: "使用量ソース",
    tokenSourceUpstream: "上流",
    tokenSourceEstimated: "推定",
  },
  settings: {
    title: "設定",
    general: "一般",
  ...
  proxyPool: {
    add: "プロキシ追加",
    edit: "プロキシ編集",
    gatewayProxies: "ゲートウェイプロキシ",
    gatewaySectionHint:
      "内蔵 BetterClaude ノードは上流 API をゲートウェイプレフィックス経由でルーティングします。手動プロキシとは同時に使用しません",
    activeGateway: "使用中",
    activeGatewayValue: "使用中：{label}（{url}）",
    noGatewayAvailable: "利用可能なゲートウェイノードがありません",
    manualProxies: "手動プロキシ",
    manualSectionHint: "個別に追加したプロキシは従来の検査と選択方法を使用します",
    name: "名前",
    namePlaceholder: "プロキシ名を入力",
    nameRequired: "プロキシ名を入力してください",
    url: "プロキシURL",
    urlPlaceholder: "http://127.0.0.1:7890",
    urlRequired: "プロキシURLを入力してください",
    invalidUrl: "http、https、socks5 のプロキシURLのみ対応しています",
    protocol: "プロトコル",
    available: "利用可能",
    bestProxy: "最速プロキシ",
    noAvailableProxy: "利用可能な検査結果なし",
    details: "詳細",
    country: "国",
    autoUpdate: "自動更新",
    minutes: "分",
    loadFailed: "プロキシプールの読み込みに失敗しました",
    deleteConfirm: "プロキシ「{name}」を削除しますか？",
    test: "プロキシテスト",
    testAll: "すべて検査",
    testCurrentPage: "現在ページを検査",
    testSettings: "検査設定",
    proxyTestSettings: "プロキシ検査",
    gatewayTestSettings: "ゲートウェイ検査",
    testTarget: "検査ターゲット",
    invalidTestTarget: "有効な HTTP/HTTPS 検査ターゲットを入力してください",
    autoTest: "定期検査",
    autoTestInterval: "検査間隔（分）",
    autoTestIntervalHours: "{hours}時間",
    autoTestIntervalMinutes: "{minutes}分",
    testTimeout: "テストタイムアウト（秒）",
    testTimeoutValue: "{seconds}秒",
    testTimeoutInline: "タイムアウト {timeout}",
    testStatus: "検査状態",
    notTested: "未検査",
    testing: "検査中",
    testPassedShort: "正常 · {duration}ms",
    testFailedShort: "失敗",
    testSuccess: "プロキシに接続できました（{duration}ms）",
    testFailed: "プロキシを利用できません：{error}",
    testRequestFailed: "プロキシ検査リクエストに失敗しました",
    testGateway: "ゲートウェイ検査",
    gatewayTestSuccess: "ゲートウェイに接続できました（{duration}ms）",
    gatewayTestFailed: "ゲートウェイを利用できません：{error}",
    testAllFinished: "プロキシ検査が完了しました",
    settingsSaved: "プロキシプール検査設定を保存しました",
    settingsLoadFailed: "プロキシプール検査設定の読み込みに失敗しました",
    settingsSaveFailed: "プロキシプール検査設定の保存に失敗しました",
    totalRecords: "{total} 件のプロキシ",
    calculatingTotal: "総数を計算中",
    pageInfo: "{current} / {total} ページ",
    pageInfoUnknown: "{current} ページ",
    recordsPerPage: "{count}件/ページ",
    unknownError: "不明なエラー",
  },
  siteManagement: {
    ...
    autoBalance: "自動残高",
    autoBalanceConfig: "自動残高設定",
    autoBalanceConfigTooltip:
      "サイト管理タイムゾーンの 00:00 を基準に、指定した時間間隔で残高を更新します",
    authType: "認証タイプ",
    authValue: "認証情報",
