...
const panel = readFileSync("src/features/site-management/components/SiteManagementPanel.vue", "utf8");
...
test("Sub2API access token auth has a separate refresh token input", () => {
  ...
  assert.match(panel, /siteForm\.site_type === 'sub2api' && siteForm\.auth_type\.includes\('access_token'\)/);
...
test("Sub2API submit serializes refresh_token with access token", () => {
  ...
  assert.match(panel, /siteForm\.auth_type\.length === 1 && !authValues\.refresh_token/);
});
...
test("Sub2API locale hints tell users where auth_token and refresh_token come from", () => {
  for (const locale of [zhSiteLocale, enSiteLocale, jaSiteLocale]) {
    ...
    assert.match(locale, /Local Storage/);
  }
});
