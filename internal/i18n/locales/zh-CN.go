...
var MessagesZhCN = map[string]string{
	...
	"validation.invalid_health_reset_interval":               "健康度重置周期无效",
	...
	"dashboard.totalTokens":                "总Token",
	"dashboard.inputTokens":                "输入Token",
	"dashboard.outputTokens":               "输出Token",
	...
	"config.proxy_pool_test_timeout_seconds_desc":          "单个代理池连通性检测请求的最长等待时间（秒）。",
	"config.proxy_pool_auto_test_interval_minutes":         "代理池定期检测间隔（分钟）",
	"config.proxy_pool_auto_test_interval_minutes_desc":    "代理池页面开启定期检测后的自动检测间隔（分钟）。",
	"config.gateway_proxy_test_timeout_seconds":            "网关代理检测超时（秒）",
	"config.gateway_proxy_test_timeout_seconds_desc":       "单次网关代理 TCP 连通性检测的最长等待时间（秒）。",
	"config.gateway_proxy_auto_test_interval_minutes":      "网关代理自动检测间隔（分钟）",
	"config.gateway_proxy_auto_test_interval_minutes_desc": "网关节点自动检测和优选的间隔（分钟）。",
	...
	"config.category.proxy_pool": "代理池",
	...
	"error.child_group_sync_failed":                     "同步子分组与父分组的更改失败",
