...
var MessagesZhCN = map[string]string{
	...
	"validation.sub_group_weight_max_exceeded":                 "子分组权重不能超过5000",
	"validation.sub_group_min_effective_weight_negative":       "最低有效权重不能为负数",
	"validation.sub_group_min_effective_weight_max_exceeded":   "最低有效权重不能超过5000",
	"validation.sub_group_min_effective_weight_exceeds_weight": "最低有效权重不能超过基础权重",
	...
	"validation.codex_support_cannot_disable_used_by_codex":    "无法关闭 Codex 支持，该分组正被以下 OpenAI Responses 聚合分组使用：{{.groups}}。请先从这些聚合分组中移除此分组，然后再关闭 Codex 支持",
	"validation.force_cc_codex_mutually_exclusive":             "同一个分组不能同时开启强制 CC 和强制 Codex",
	...
	"dashboard.cacheTokens":                "缓存 Tokens",
	"dashboard.thinkingTokens":             "思考 Tokens",
	"dashboard.estimatedTokens":            "估算 Tokens",
