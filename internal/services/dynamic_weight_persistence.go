...
// NOTE: Non-digit characters are silently stripped (not an error) for defensive parsing.
// Example: "12a34" returns 1234, not an error.
// This behavior is intentional for robustness when parsing internally-generated keys.
// Keys are generated by SubGroupMetricsKey and ModelRedirectMetricsKey with guaranteed format.
// Invalid keys (if any due to data corruption) will be filtered out when keyToDBMetric returns nil.
// Using strict parsing (strconv.ParseUint) was considered but rejected as over-engineering.
