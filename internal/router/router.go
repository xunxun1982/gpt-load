...
func registerProtectedAPIRoutes(api *gin.RouterGroup, serverHandler *handler.Server, configManager types.ConfigManager) {
	...
	groups := api.Group("/groups")
	{
		...
		groups.POST("/:id/sub-groups-health/reset", serverHandler.ResetAllSubGroupHealth)
	...
	dashboard := api.Group("/dashboard")
	{
		...
		dashboard.GET("/token-usage", serverHandler.TokenUsage)
	...
	proxyPool := api.Group("/proxy-pool")
	{
		...
		proxyPool.GET("/gateway-options", serverHandler.ListGatewayProxyOptions)
		proxyPool.POST("/gateway-options/:candidate_id/test", serverHandler.TestGatewayProxy)
	...
	siteMgmt := api.Group("/site-management")
	{
		...
		siteMgmt.PUT("/reorder", serverHandler.ReorderManagedSites)
