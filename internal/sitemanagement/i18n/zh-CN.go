...
var MessagesZhCN = map[string]string{
	...
	"site_management.validation.invalid_bypass_method":           "无效的绕过方式",
	...
	"site_management.validation.auth_changed_retry":              "站点认证信息已变化，请刷新后重试",
	"site_management.validation.configuration_changed_retry":     "站点配置已变化，请刷新后重试",
	"site_management.validation.anyrouter_requires_cookie":       "AnyRouter 只能使用 Cookie 认证",
	"site_management.validation.anyrouter_user_id_required":      "启用 AnyRouter 自动功能时必须填写用户 ID",
	"site_management.validation.anyrouter_cookie_required":       "启用 AnyRouter 自动功能时必须填写 Cookie",
	"site_management.validation.stealth_requires_cookie":         "隐身绕过必须配置 Cookie 认证",
	"site_management.validation.stealth_cookie_required":         "隐身绕过必须填写 Cookie",
	"site_management.validation.stealth_waf_cookie_required":     "隐身绕过的 Cookie 至少要包含一个有效的浏览器防护 Cookie：{{.cookies}}",
	"site_management.validation.sub2api_requires_access_token":   "启用 Sub2API 自动功能时必须选择 Access Token 认证",
	"site_management.validation.sub2api_credential_required":     "启用 Sub2API 自动功能时必须填写 Access Token 或 Refresh Token",
	"site_management.validation.sub2api_custom_checkin_required": "Sub2API 上游没有内置签到，启用自动签到前必须填写自定义签到地址",
