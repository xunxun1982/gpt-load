...
var MessagesJaJP = map[string]string{
	...
	"site_management.validation.invalid_bypass_method":           "無効なバイパス方式",
	...
	"site_management.validation.auth_changed_retry":              "サイトの認証情報が変更されました。再読み込みしてもう一度お試しください",
	"site_management.validation.configuration_changed_retry":     "サイト設定が変更されました。再読み込みしてもう一度お試しください",
	"site_management.validation.anyrouter_requires_cookie":       "AnyRouter は Cookie 認証のみ対応しています",
	"site_management.validation.anyrouter_user_id_required":      "AnyRouter の自動処理を有効にするにはユーザー ID が必要です",
	"site_management.validation.anyrouter_cookie_required":       "AnyRouter の自動処理を有効にするには Cookie が必要です",
	"site_management.validation.stealth_requires_cookie":         "ステルスバイパスには Cookie 認証が必要です",
	"site_management.validation.stealth_cookie_required":         "ステルスバイパスには Cookie 値が必要です",
	"site_management.validation.stealth_waf_cookie_required":     "ステルスバイパスには少なくとも 1 つのブラウザ保護 Cookie が必要です: {{.cookies}}",
	"site_management.validation.sub2api_requires_access_token":   "Sub2API の自動処理を有効にするには Access Token 認証が必要です",
	"site_management.validation.sub2api_credential_required":     "Sub2API の自動処理を有効にするには Access Token または Refresh Token が必要です",
	"site_management.validation.sub2api_custom_checkin_required": "Sub2API 上流には組み込みチェックインがありません。自動チェックインを有効にする前にカスタム URL を設定してください",
